...
@set ProjectSource=c:\src\projects\cpp\shell\hextab
@set ProjectName=hextab
@set ProjectCompany=mtanabe
@set ProductCode={6972593B-7F1E-420E-8F5D-32ABBF1318CF}
@set ProductName=Maximilian's Hex Tab for Windows Explorer
@set ProductDescription=This setup program installs a Windows Explorer add-on. It adds a custom hex-dump tab to Windows Explorer File Properties. You need administrator privileges to install.
...
set MTPLATFORM=x64
if "%1"=="x64" (
"%PSDKBIN%\signtool.exe" sign /v /s "%CERT_STORE%" /n "%SIGNING_CERT%" /t http://timestamp.comodoca.com/authenticode  .\x64\release\hextab.dll
@if errorlevel 1 goto exitBatch
"%PSDKBIN%\signtool.exe" sign /v /s "%CERT_STORE%" /n "%SIGNING_CERT%" /t http://timestamp.comodoca.com/authenticode  .\x64\release\hexdlg.exe
@if errorlevel 1 goto exitBatch
"%PSDKBIN%\signtool.exe" sign /v /s "%CERT_STORE%" /n "%SIGNING_CERT%" /t http://timestamp.comodoca.com/authenticode  .\x64\release\scansrvwebp.dll
@if errorlevel 1 goto exitBatch
"%PSDKBIN%\msiinfo.exe" %ProjectName%.msi /p x64;1033
goto runMakeMsi
)
if not "%1"=="x86" (
...
"%PSDKBIN%\signtool.exe" sign /v /s "%CERT_STORE%" /n "%SIGNING_CERT%" /t http://timestamp.comodoca.com/authenticode  .\release\hextab.dll
@if errorlevel 1 goto exitBatch
"%PSDKBIN%\signtool.exe" sign /v /s "%CERT_STORE%" /n "%SIGNING_CERT%" /t http://timestamp.comodoca.com/authenticode  .\release\hexdlg.exe
@if errorlevel 1 goto exitBatch
"%PSDKBIN%\signtool.exe" sign /v /s "%CERT_STORE%" /n "%SIGNING_CERT%" /t http://timestamp.comodoca.com/authenticode  .\release\scansrvwebp.dll
@if errorlevel 1 goto exitBatch
set MTPLATFORM=x86
...
attrib -r %ProjectSource%\installer\out\%ProjectName%64.msi
...
attrib -r %ProjectSource%\installer\out\%ProjectName%86.msi
